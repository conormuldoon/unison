# Unison
Unison is a service that enables (HARMONIE-AROME) numerical weather forecast data from meteorological services to be tracked and visualised through a web interface and historical data to be accessed via a RESTful API. The version of the code in this repository (deployed at https://aqua.ucd.ie/unison/) has been configured for use with Met Éireann's model (coverage of Ireland and the UK), but it can also be used with the Norwegian Meteorological Institute's API or other APIs that conform to the same schema. Unison has been developed using Spring Boot, JPA, and React and can be configured for use with either an embedded GeoDB geospatial database or Postgres/PostGIS for a production environment.
...
```
java -cp target/unison-server-0.3.0-SNAPSHOT.jar -Dloader.main=eu.acclimatize.unison.user.UserConsole org.springframework.boot.loader.PropertiesLauncher
```
...
The database configuration is specified in the `back-end/src/main/resources/application.properties` file. Comment out the properties related GeoDB and uncomment the properties related to Postgres/PostGIS or vice versa and restart the server. Alternatively, to automate the process (for instance, for continuous integration purposes), use the Python script to comment out and uncomment the relevant properties:
```
cd back-end
